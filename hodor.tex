...
is serialized before its parent, and each node is serialized immediately after
...
order. This way, each node will fall after all of its ancestors [FIGURE 2].
Then, if we traverse downwards from a node, as we do in a range query, we will
always read forward through the file. 
