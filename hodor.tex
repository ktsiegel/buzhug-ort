...
its linked subtree in the next dimension. This forms a series of levels
\cite{seinfeld} for each tree [FIGURE 1]. Consider a BORT, $t$, which is indexed
in dimension $d$. Each node $n$ in $t$ falls in the same level as all of its
siblings. The entirety of node's linked subtree, $t_n$, which will be in the
next dimension ($d - 1$), also falls in this same level relative to $t$. All of
$n$'s children and their linked subtrees fall in the level below, and $n$'s
parent falls in the level above. 
