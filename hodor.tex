...
are the B Tree, and the modified version, the B+ Tree. As a quick overview, a B
...
optimizations like line iterator or linecache features \cite{linecache}.
...
and a \textsc{GetRange} call. \textsc{GetRange} is a function which returns all
leaves in a range from this node's subtree. It is accomplished by starting from
the leaf containing the maximum value and walking backwards along the leaf's
\ms{prev} pointers until the leaf containing the minimum value is found. This
entire operation takes $O(\log_B m + k)$ reads: the search down the tree
touches $\log_B m$ nodes, and the walk touches $k$ leaves.
...
The logic at the leaf level is shown in Algorithm 2. Once \textsc{RangeQuery} is
...
complexity jumps to $O(\log_B m + k)$ reads. 
...
As discussed in section 2.4, the nodes are structured into recursive levels on disk,
...
illustrated by node 2. Suppose node 2's entire range is contained in the queried
interval, so we recurse on its linked node, node 5. Note that the linked
...
level. Thus, a recursive call of the first case costs nothing in back seeks.

The child node recursion case can be illustrated by node 1, in level 1. Suppose
that node 1 contains \ms{start}, so we recurse on node 1 and load its children
in 0. Level 1 comes before level 0 on disk, so we can load node 1's children by
seeking forward. However, if the range query ever has to access level 1 again, a
back seek will be needed to return from level 0. This happens whenever the
recursive call on a node is followed by a recursive call on one of that node's
siblings. For example, the recursive call on node 1 will traverse down the tree,
ending at some level below 1 in the tree file. If we next recurse on one of the
nodes 2-4, it will cost one back seek to return back to level 1. In other words,
during the recursion on node 0, if more than one recursive call of the second
case is made, then every call but the last will contribute one back seek. \\
...
\begin{proof}
    ...
    second case (child node recursion) that is followed by another recursive
    call in the same step contributes one back seek. A recursive call of the
    second case can only be called on a child whose range is not completely
    contained within the queried interval, and whose range contains the
    \ms{start} or \ms{end} point. So, for each recursive step, there will be at
    most two recursive calls of the second case. If there is zero or one, then
    the recursive step does not contribute any back seeks. If there are two, the
    recursive step contributes exactly one back seek.
    ...
    \begin{figure}[h!]
        ...
        \caption{
            ...
            including 0, 1, 3 and 4, require only one recursive call on each
            node's child.
        }
        \vspace{0.5in}
    \end{figure}
    ...
    that is an ancestor of \ms{lca} (nodes 0 and 1) covers the range of
    ...
    in the second dimension. Recursing on $d$, we get $O(\log_B^{d-1}n)$ for $d$
    dimensions.
...
accomplishes this and analyze its complexity in back seeks.
...
With a naive recursive range
query implementation, it is possible to
achieve $O(\log_B^d n)$ back seeks by using a similarly recursive disk
