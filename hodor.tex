...
query accesses the root's children, it does all of its work in the same level.
Each level is sorted in reverse order in the disk layout, so if the range query
...
are two possible types of recursive calls. Either we recurse on a child's
linked subtree, or we recurse on the child itself. In the first case, since
each child's linked subtree falls in the same level as the child and comes
immediately after the child in the disk layout, the recursive call stays within
the same level as the child. In the second case...
