...
back seeks. Here we review the typical range query implementation and analyze
its worst-case back seek count. 

A range query algorithm works recursively as it traverses down a tree in some
dimension, treating each node as the root of its subtree. For each of the
root's children, the range query determines if the child's range lies
completely within the desired range. If it does, then the algorithm recurses on
the child's linked node in the next dimension. Otherwise, the algorithm
recurses on the child itself. 

Recall that the nodes are structured into recursive levels on disk, where each
level contains a node, all of its siblings, and its linked subtree in the next
dimension. The number of back seeks we do during each recursion depends on the
order in which nodes of the same level are accessed and the order in which the
levels themselves are accessed. Each level is sorted in reverse order, so if a
node is accessed after a left sibling, it will cost one back seek. For each
tree, the levels themselves are sorted in order from highest to lowest, so if
an ancestor is accessed after the node itself, it will cost one back seek. 

Because a node and all of its siblings lie in the same level, when the range
query accesses the root's children, it does all of its work in one level. Each
...
There are two possible types of recursive calls: either we recurse on a node's
linked subtree, or we recurse on the node itself. In the first case, note that
the entirety of each node's linked subtree falls in the same level as the node
and comes immediately after the in the disk layout. Therefore, the
recursive call stays within the same level as the node, and the file pointer
seeks forward. In the second case, each of the node's children is located in
the next level of the file, so the file pointer jumps forward and recurses on
each.

%Say something about querying linked subtrees before children here
