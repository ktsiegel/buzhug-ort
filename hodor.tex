...
Tree serialization is executed while preprocessing the datapoints into a tree, so
that the order of nodes in the tree file is the same as the order in which we
preprocess them. Building the tree is done recursively, from the bottom up, and
returns the root's tree file pointer. We start with a set of children. If these
are leaves, then we sort them on their values. Next, we determine their parents
by partitioning the children into clusters of size $B$. For each of these
...
since the single parent is the root, so we return its pointer.  Otherwise, we
use the parents as the set of children in the next recursive call. 
