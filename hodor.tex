...
% move fig 1 here
Hodor serializes the nodes in the same order as they are created. In the
recursion, we serialize the children, the linked subtrees, and finally the
parents. Each child is serialized before its parent. Each node is serialized
immediately after its linked subtree in the next dimension. This forms a series
of levels, where each node and its linked subtree fall in the same level, and
the node's children and parent fall in the levels before and after,
respectively [FIGURE 1]. The leaves of the tree in the first dimension are in
the first level, at the head of the file, and the root of the same tree is in
the last level, the last node in the file. 

% move fig 2 here
Once we finish building the tree, we flush the file contents to disk in reverse
order. This way, each node will fall after all of its ancestors [FIGURE 2]. Then, if we
traverse downwards from a node, as we do in a range query, we will always read
